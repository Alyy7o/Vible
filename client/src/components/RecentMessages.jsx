        ...
        <div className="flex flex-col max-h-56 overflow-y-scroll no-scrollbar">
            {
                messages.map((message, index) => (
                    <Link to={`/messages/${message.from_user_id._id}`} key={index} className='flex items-start gap-2 p-2 hover:bg-slate-100'>
                        <img src={message.from_user_id.profile_picture} className='w-8 h-8 rounded-full' alt="" />

                        <div className="w-full">
                            <div className="flex justify-between">
                                <p className='font-medium'>{ message.from_user_id.full_name }</p>
                                <p className='text-slate-400 text-[10px]'>{ moment(message.createAt).fromNow() }</p>
                            </div>
